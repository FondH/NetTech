...
DWORD WINAPI tnsThrd(LPVOID lpParam) {
    ...
    while (keep_trn_trd) {
        ...
        string opt = "Error: ";
        if (rst == 1)
            opt += intToIp(ntohl(v4head->destination_address)) + " ·�ɲ�ѯʧ��";
        else if (rst == 2) {
            opt += intToIp(ntohl(v4head->destination_address))+ ":" + arrayToMac(((eth_header*)pkt)->dst_mac) + "��һ����ַ��ѯʧ��";
            //Send ICMP Timeout
        }
        else if (rst == 3) {
            opt += intToIp(ntohl(v4head->destination_address)) + ":" + arrayToMac(((eth_header*)pkt)->dst_mac) + "INC ��������ʧ��";
        }
        logger.push(opt);
